...
## アプリケーションの用意
gitを使うにあたって、必要なアプリケーションを二つ、用意します。

一つは、**Visual Studio Code** (以下VSCode)です。VSCodeは、エディタ（ソースコードを書くためのソフト）として極めて優秀ですが、ここでは、gitの管理用に用意します。[VSCodeの使い方](../vscode.qmd)を参考に、インストールします。

もう一つは、gitを操作するために必要な**Git**アプリです。[公式Webサイト](https://git-scm.com/)からインストーラーをダウンロードしてきてもよいですが、以下のような`winget`（Windows）あるいは`brew`（macOS）を使ったインストールが簡単です。
